      ...
      <div className='flex justify-center gap-4 sm:gap-8 my-10 relative'>
        {blogCategories.map((item) =>(
            <div key={item} className='relative'>
                {/* we use state to change the colour on select */}
                <button onClick={()=> setMenu(item)} className={`cursor-pointer text-gray-500 ${menu == item && 'text-white px-4 pt-0.5'}`}>
                    {item}
                    {menu==item && (
                        <motion.div layoutId='underline'
                        transition={{type:'spring',stifness:500,damping:30}} className='absolute left-0 right-0 top-0 h-7 -z-1 bg-primary rounded-full'>
